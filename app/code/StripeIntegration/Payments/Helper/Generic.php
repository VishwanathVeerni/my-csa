...
class Generic
{
    ...
    public function convertPaymentMethodToCard(\Stripe\PaymentMethod $paymentMethod)
    {
        ...
        if (!empty($data['payment_element']))
        {
            ...
            if ($token)
            {
                ...
                if ($config->reCheckCVCForSavedCards())
                {
                    ...
                    $paymentMethod = $config->getStripeClient()->paymentMethods->retrieve($token, []);
