...
class WebhooksSetup
{
    ...
    public function isConfigureNeeded()
    {
        ...
        if ($automaticConfigurationEnabled == 0)
        {
            return false;
        }
        ...
        foreach ($keys as $secretKey => $publishableKey)
        {
            ...
            if ($webhookModel->getConfigVersion() != WebhooksSetup::VERSION)
            {
                return true;
            }

            if (!in_array($webhookModel->getUrl(), $urls))
            {
                return true;
            }

            $enabledEvents = json_decode($webhookModel->getEnabledEvents(), true);

            if (!$this->compare->areArrayValuesTheSame($enabledEvents, WebhooksSetup::$enabledEvents))
            {
                return true;
            }
        }
